...
import java.io.*;
import java.util.*;
//import utils.validation.transcripts.RunJob_Antonio;
...
public class RegionQuantifier
{
	private static BufferedWriter writer;
	private static Timer timer;
	static final String USAGE = "java fps.RegionQuantifier <List of SNPs file> <FASTA file> <Truncate name at space character? true | false> <BAM file> <Read length of the simulated dataset> <outputFile> <Run BLAST against any specific database? true | false> <Target BLAST database file | none>";
...
// -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	private static void quantify(File listOfSNPsFile, File fastaFile, boolean truncateNameAtSpaceChar, File sortedBAMFile, int readLengthSimulation) throws IOException
	{
		...
		for (String contigName : snpLookup.keySet())
		{
			...
			//RunJob_Antonio.runJob(com4Faidx);
			...
			//RunJob_Antonio.runJob(com4ExtractContigFromBAMFile);
			...
			//RunJob_Antonio.runJob(comSAM2BAM);
			...
			//RunJob_Antonio.runJob(comIndexBAM);
			...
			for (Integer snpPos : snpLookup.get(contigName))
			{
				...
				// ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				alleleExtractorLight(new File(convBAMFile.getAbsolutePath()), snpPos.intValue(), faidx, baiFile, contigName, refAlleleAtPos, finalPosOfContig, snpReport, snpReport2, contigFastaFile, chrmLookup);
		...
		//RunJob_Antonio.runJob(cmd2DeleteSAMs);
...
// -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	private static void alleleExtractorLight(File contigNameSortedBAMFile, int snpPos, IndexedFastaSequenceFile faidx, File baiFile, String contigName, char refAlleleAtPos, int finalPosOfContig, SNPReport snpReport, SNPReport2 snpReport2, File contigFastaFile, HashMap<String, String> chrmLookup)
	{
		...
		try
		{
			...
			if (databaseFile != null)
			{
				// Inserted this check in order to avoid repeating the BLAST for each SNP position within a contig...
				// ################################################################################################################################################################################################################################################################################################
				if (! new File(contigName + "_contig_vs_dbFile.txt").isFile()) 
				{
					if (debug)
						System.out.println("\nBLASTing the assembled contig " + contigName + " against the dbFile " + databaseFile + " to retrieve the original genomic coordinates of the FPSNP site...");
					String com4IntermediateBLAST = "/mnt/shared/bio_software/ncbi-blast-2.2.28+/bin/blastn -task blastn -query " + contigFastaFile.getAbsolutePath() + " -db " + databaseFile + " -max_hsps_per_subject 1 -outfmt 6 -ungapped -max_target_seqs 1 -out " + contigName + "_contig_vs_dbFile.txt";
					new RunJob_Antonio().runJob(com4IntermediateBLAST);
					String [] runJobClassOutputs = new RunJob_Antonio().runJob(com4IntermediateBLAST);
					String runJobClassStdOut = runJobClassOutputs[0];
					String runJobClassStdErr = runJobClassOutputs[1];
					System.out.println("runJobClassStdOut = "+runJobClassStdOut);
					System.out.println("runJobClassStdErr = "+runJobClassStdErr);
				...
				// #################################################################################################################################################################################################################################################################################################
				File blastResults4ContigAndSNPposition = new File(contigName + "_contig_vs_dbFile.txt");
				if (debug)
					System.out.println("\nReading the " + contigName + "_contig_vs_dbFile.txt file");
																								
				// ------------------------------------ Block to try to capture the info whether the Blast resulting file is EMPTY or not prior to its real reading/access attempt... -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				BufferedReader br = new BufferedReader(new FileReader(blastResults4ContigAndSNPposition));
				if (br.readLine() == null) 
				{
					if (debug)
						System.out.println("\n:o( ... WARNING: Not so good...!!! Resulting file " + contigName + "_contig_vs_dbFile.txt seems to be EMPTY... :o( Please, check it !!!...");
					blastResultIsEmpty = true;
					accumNotPossible2DetermineSNPPosOnGenome++;
				}
				// -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				br = null;
				try
				{
					String line;
					br = new BufferedReader(new FileReader(blastResults4ContigAndSNPposition));
					while ((line = br.readLine()) != null)
					{
						if (debug)
							System.out.println("\nI've entered in the branch where the line of the blast result file IS NOT NULL!!!");
						String[] lineArray;
						if (line.startsWith(contigName))
						{
							if (debug)
								System.out.println("\n:o) ... Seems promising! Resulting file " + contigName + "_contig_vs_dbFile.txt IS NOT EMPTY... :o) ...");
							lineArray = line.split("\t");
							chrmInGenome = lineArray[1];
							if (debug)
								System.out.println("\nOriginal chromosome in genome is: " + chrmInGenome + ".");
							sStart = Integer.parseInt(lineArray[8]);
							if (debug)
								System.out.println("\nContig START position in genome is: " + sStart + ".");
							sEnd = Integer.parseInt(lineArray[9]);
							if (debug)
				...
				if (debug)
					System.out.println("\nEXITING the 'BLAST of assembled contig against the dbFile to retrieve the original genomic coordinates of the FPSNP site' substep...");
				//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
				if (debug)
